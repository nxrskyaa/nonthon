...
/**
 * URL-encode a string the way the upstream does: encodeURIComponent plus the
 * four chars encodeURIComponent leaves alone ( ! ' ( ) ) escaped as %XX.
 * Mirrors the `wT()` helper in the current web player bundle.
 */
function urlEncode(s) {
    return encodeURIComponent(s).replace(/[!'()]/g, t => '%' + t.charCodeAt(0).toString(16).toUpperCase());
}

/** JSON-ish fallback for non-string, non-array values (numbers, booleans, nested objects). */
function jsonify(v) {
    return typeof v === 'string' ? v : v == null ? 'null' : JSON.stringify(v);
}

/**
 * Serialize one plain object to the canonical string used for signing.
 * Mirrors the `oc()` helper in the current web player bundle:
 * - sort keys, for each key:
 *   - string  -> URL-encoded value (empty string stays empty)
 *   - array   -> join items (strings raw, objects recursed, others JSON)
 *   - else    -> jsonify()
 */
function serializeObject(obj) {
    return Object.keys(obj).sort().map(k => {
        const v = obj[k];
        if (typeof v === 'string') return v ? urlEncode(v) : '';
        if (Array.isArray(v)) {
            return v.map(item =>
                typeof item === 'string' ? (item || '') :
                item && typeof item === 'object' && !Array.isArray(item) ? serializeObject(item) :
                jsonify(item)
            ).join('');
        }
        return jsonify(v);
 ...
 * This matches the `Im()` function in the current web player bundle:
 * - objects  -> serializeObject()
 * - arrays   -> filter to plain objects, serialize each, join
 * - strings  -> try to parse as JSON and re-serialize
 * Note: string values are URL-encoded (the upstream changed this — raw values
 * now produce "sign error!" from the API).
 */
